...
# Addendum — a ramp cell can also carry any other lateral connection

A Tunnel Ramp used to require the cell on each side to already be (and stay)
a clean straight stretch of track — the old `directionBlockedByRamp`
capped a ramp cell's OTHER connections at exactly its own straight-through
pair, both at ramp-build time (`buildUndergroundRamp`'s old
`onlyStraightThrough` precondition) and afterward (auto-connect/manual
Connect). That cap is removed: a ramp cell can now carry any of the 6
pairwise lateral direction combinations (N-S/E-W/N-E/N-W/S-E/S-W) — up to
and including a full 4-way rail crossing — at the same time as a Tunnel
Ramp, or a same-cell vertical Ramp, or both.

## Design

- **Deciding what's buildable and deciding what's actually travelable are
  now two separate questions**, and the second one already had a correct
  answer once § Rail crossings existed: `findLayerPath`'s `straightOnly`
  restriction at a genuine 4-way crossing is what makes "no turning onto
  the other line" true, regardless of how the cell's edges got built. The
  old build-time cap was solving the same problem the crude way, before
  that finer mechanism existed — removing it doesn't reopen any hole, since
  pathfinding was already the real gatekeeper.
- **The Tunnel Ramp's lateral move is now subject to the SAME
  `straightOnly` filter as the ordinary lateral edges.** Descending into a
  tunnel is "continuing this line, just sloping down" — so at a crossing
  cell, the ramp is only reachable from the line whose straight-through
  direction it actually sits on, exactly like any other lateral exit. A
  same-cell VERTICAL Ramp (ground&lt;-&gt;elevated, same x,y different
  layer) was never subject to this at all — it's not a lateral move, so it
  needed no change; it already worked at a crossing cell with zero code.
- **T/3-way junctions were never capped by this rule and stay exactly as
  permissive as before** — `directionBlockedByRamp` only ever fired once a
  rampEdge existed, and removing it doesn't change anything about ordinary
  (ramp-free) junctions.

## What changed

- **`commands.js`**: removed `directionBlockedByRamp` and its 4 call sites
  (2 in `connectNewTileEdges`, 2 in `cmdToggleConnection`); removed
  `buildUndergroundRamp`'s `onlyStraightThrough` precondition and its
  rejection warning.
- **`pathfinding.js`**: `findLayerPath`'s lateral ramp-edge loop now checks
  the same `straightOnly` filter the ordinary lateral-edges loop already
  uses.
- **`ui.js`**: dropped the now-inaccurate "only works along a straight
  stretch" claim from the Tunnel Ramp/Rail Tunnel Ramp tool hint text.

## Testing

`test-underground.js` Test 3 and Test 4, and `test-multilevel-tunnel.js`
Test 3, were rewritten from "rejects a perpendicular connection at a ramp
cell" to "accepts one" — the old rejection was the exact behavior being
removed. New Test 5/6 in `test-rail-crossing.js` (10 checks) cover the
combination directly: a Tunnel Ramp builds with no rejection at a busy
4-way crossing cell, the crossing's own straight-through behavior is
unaffected by the ramp also being there, descending into the tunnel from
the wrong (perpendicular) line is still blocked, descending from the line
that actually leads there is allowed, and — as a pure regression check — a
same-cell vertical Ramp at a crossing cell needs no restriction at all
since it never had one. All 13 test files pass. Verified end-to-end in a
real browser via Playwright, driving the actual `postCommand` pipeline (not
a shortcut): built a live crossing, added an underground tile and a Tunnel
Ramp through it with zero warnings, and confirmed both the crossing's 4
connections and the new ramp edge coexist correctly — visually confirmed
the crossing still renders as a clean "X" with the ramp's diamond marker
sitting at the same cell, zero console errors.
