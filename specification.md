...
>
> **Further amended in Phase 2 (Multi-level tunnels).** Underground is no
> longer one grade — `UNDERGROUND_LEVELS` (default 3) stacks that many,
> each reached from the one above it (ground, for level 1) by its own
> Tunnel Ramp, never skipping a level, and costing progressively more the
> deeper it goes. Level 1 is a pure extension of what already existed
> (`undergroundGradeName(1)` resolves to the original `'underground'`
> name, so nothing needed migrating), generated by the same loop-based
> pattern `LAYER_GRADE_KIND`/`GRADE_KIND_LAYER`/a cell's `layers` already
> used for deepUnderground/airspace. The one real design change: a track's
> `rampEdge[dir]` used to be a boolean, safe only because there was
> exactly one possible pair (ground↔underground) it could ever mean. With
> a stack, a single cell partway down can have a ramp edge going up one
> level in one direction and a different one going down a level in
> another, so `rampEdge[dir]` now stores the actual grade name it leads
> to. That change simplified more than it complicated — `cmdDemolish`'s
> ramp cleanup no longer guesses "the other grade" from a hardcoded swap,
> it just reads the stored name directly, which works identically for any
> pair. The straight-through constraint that keeps a ramp off of turns and
> junctions composes across any number of chained levels with no changes
> at all: each ramp's own axis is exactly the allowed continuation
> direction for the next ramp down, the same relationship that already
> held for the single-level case. As with every prior amendment to this
